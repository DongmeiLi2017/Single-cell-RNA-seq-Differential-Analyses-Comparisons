...
for (j in 1:length(diffpro))
{
  ...
  while (num<=total)
  {
    ...
    simulateddata <- RnaXSim(expMatrix, distribution="LogNorm", NumRep=trtsize, NumDiff = ndiff[j], NumFea = ngene)
    truev <- c(rep(0, ngene))
...
write.csv(meanFDR, file="D:/Box Sync/Conferences/Huston_2018/Simulation_Results/meanFDR_DESeq2_n_100_propzero_LogNorm_Islam2011.csv")
write.csv(meansen, file="D:/Box Sync/Conferences/Huston_2018/Simulation_Results/meansen_DESeq2_n_100_propzero_LogNorm_Islam2011.csv")
write.csv(meanspe, file="D:/Box Sync/Conferences/Huston_2018/Simulation_Results/meanspe_DESeq2_n_100_propzero_LogNorm_Islam2011.csv")
write.csv(meanacu, file="D:/Box Sync/Conferences/Huston_2018/Simulation_Results/meanacu_DESeq2_n_100_propzero_LogNorm_Islam2011.csv")
write.csv(meanauc, file="D:/Box Sync/Conferences/Huston_2018/Simulation_Results/meanauc_DESeq2_n_100_propzero_LogNorm_Islam2011.csv")
...
g_range<-range(meanFDR[, -1], na.rm=TRUE)
...
g_range<-range(meansen[, -1], na.rm=TRUE)
...
g_range<-range(meanspe[, -1], na.rm=TRUE)
...
g_range<-range(meanacu[, -1], na.rm=TRUE)
...
g_range<-range(meanauc[, -1], na.rm=TRUE)
...
legend("left", c("DEsingle","Linnorm", "monocle", "MAST", "DESeq2"), cex=1.0,
       col=c("blue","red", "darkgreen", "black", "orange"), lwd=2, lty=c(1:4, 1:4), bty="n")

dev.off()

diffpro<-c(0.05, 0.10, 0.20, 0.30, 0.40, 0.50)
meanauc <- read.csv("D:/Box Sync/Conferences/Huston_2018/Simulation_Results/meanauc_DESeq2_n_5_propzero_Poisson.csv", header = TRUE)

dev.off()
par(mfrow=c(1, 1), cex=1.0, mgp=c(3, 1, 0), mar=c(4.5, 4.5, 1.5, 0.7))
g_range<-range(meanauc[, 3:7], na.rm=TRUE)
plot(diffpro, meanauc$aucmean.DEsingle, type="l", lty=1, lwd=2, col="blue", ylim=g_range, ann=FALSE)
box()
lines(diffpro, meanauc$aucmean.Linnorm, type="l", lty=2, lwd=2, col="red")
lines(diffpro, meanauc$aucmean.monocle, type="l", lty=3, lwd=2, col="darkgreen")
lines(diffpro, meanauc$aucmean.MAST, type="l", lty=4, lwd=2, col="black")
lines(diffpro, meanauc$aucmean.DESeq2, type="l", lty=5, lwd=2, col="orange")
title(xlab=expression(pi[1]), ylab="AUC")
legend("bottomright", c("DEsingle","Linnorm", "monocle", "MAST", "DESeq2"), cex=1.0,
       col=c("blue","red", "darkgreen", "black", "orange"), lwd=2, lty=c(1:4, 1:4), bty="n")
